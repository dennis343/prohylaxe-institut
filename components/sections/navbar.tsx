  ...
  return (
    ...
    >
      <div className="mx-auto flex h-16 max-w-6xl items-center justify-between gap-3 px-4 sm:px-5 md:h-20 md:px-8">
        ...
        >
          <span className="flex h-10 w-10 items-center justify-center rounded-full bg-primary text-primary-foreground transition-colors group-hover:bg-accent group-hover:text-accent-foreground">
            <span className="font-serif text-lg italic leading-none">M</span>
          </span>
          <span className="flex flex-col leading-tight">
            <span className="font-serif text-[14px] font-medium italic tracking-tight text-foreground sm:text-[15px] md:text-base">
              Prophylaxe-Institut
            </span>
        ...
        <button
          ...
          onClick={() => setOpen((v) => !v)}
          className="inline-flex h-11 w-11 items-center justify-center rounded-full bg-primary text-primary-foreground shadow-sm transition-colors hover:bg-accent hover:text-accent-foreground focus-visible:ring-2 focus-visible:ring-accent focus-visible:ring-offset-2 focus-visible:ring-offset-background md:hidden"
        >
      ...
      {open && (
        <div
          id="mobile-nav"
          className="fixed inset-0 top-[calc(env(safe-area-inset-top)+4rem)] z-[60] flex flex-col overflow-y-auto bg-background pb-[env(safe-area-inset-bottom)] md:hidden"
        >
          <nav
            ...
            {navLinks.map((link) => {
              ...
              return (
                <Link
                  ...
                  aria-current={active ? "page" : undefined}
                  className={cn(
                    "flex min-h-[60px] items-center justify-between gap-4 border-b border-border py-4 font-serif text-2xl tracking-tight transition-colors sm:text-3xl",
          ...
          <div className="mt-auto flex flex-col gap-3 px-5 pb-8 pt-8">
            ...
            <Button
              ...
              size="lg"
              variant="outline"
              className="w-full rounded-full border-foreground/25 text-foreground"
              onClick={() => setOpen(false)}
            >
              <Link href="/foerder-check">Förder-Check starten</Link>
            </Button>
